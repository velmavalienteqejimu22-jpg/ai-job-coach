    ...
    switch (action) {
      ...
      case "answer": {
        ...
        if (useStub) {
          ...
          const question = recentMessages?.find((m: ChatMessage) => m.role === "assistant")?.content || "";
      ...
      case "next_question": {
        ...
        if (useStub) {
          return NextResponse.json(stubNextQuestion(finalRoundType, currentIndex));
        } else {
          const result = await deepseekNextQuestion(finalRoundType, answeredQuestions, currentIndex);
          return NextResponse.json(result);
        }
      }
      ...
      case "finish_round": {
        ...
        if (useStub) {
          ...
          const result = await deepseekFinishRound(finalRoundType, allQuestions);
          return NextResponse.json(result);
        }
      }
